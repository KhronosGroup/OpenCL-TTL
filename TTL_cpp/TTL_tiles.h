 ...
 *
 * A TTL_augmented_dim is the number of elements to augment.
 */

typedef unsigned char TTL_augmented_dim;  ///< A number of a "augmented" elements in the unit of elements
...
struct TTL_augmentation {
     ...
     */
    TTL_augmentation(const TTL_augmented_dim left = 0, const TTL_augmented_dim right = 0,
                     const TTL_augmented_dim top = 0, const TTL_augmented_dim bottom = 0,
                     const TTL_augmented_dim front = 0, const TTL_augmented_dim back = 0)
        : left(left), right(right), top(top), bottom(bottom), front(front), back(back) {}

    TTL_augmented_dim left;    ///< Left hand augmentation in elements
    TTL_augmented_dim right;   ///< Right hand augmentation in elements
    TTL_augmented_dim top;     ///< Top augmentation in elements
    TTL_augmented_dim bottom;  ///< Bottom augmentation in elements
    TTL_augmented_dim front;   ///< Front augmentation in elements
    TTL_augmented_dim back;    ///< Back augmentation in elements
};
...
struct TTL_tiler {
    ...
    TTL_tiler(const TENSORSHAPETYPE tensor_shape, const TILESHAPETYPE tile_shape, const TTL_overlap overlap,
        ...
        const TTL_dim tiles_in_depth =
            TTL_ceil_of_a_div_b(tensor_shape.depth + augmentation.front + augmentation.back - overlap.depth,
                                tile_shape.depth - overlap.depth);

        const TTL_dim tiles_in_plane = tiles_in_width * tiles_in_height;
        const TTL_dim number_of_tiles = tiles_in_plane * tiles_in_depth;
    ...
    TTL_tile<TILESHAPETYPE> get_tile(const int tile_id) const {
        ...
        const TTL_dim y = tid_in_plane / cache.tiles_in_width;
        const TTL_dim x = tid_in_plane % cache.tiles_in_width;
    ...
    TTL_tile<TILESHAPETYPE> get_tile_column_major(const int tile_id) const {
        ...
        const TTL_dim z = tile_id / cache.tiles_in_plane;
        const TTL_dim tid_in_plane = tile_id % cache.tiles_in_plane;
        const TTL_dim y = tid_in_plane % cache.tiles_in_height;
        const TTL_dim x = tid_in_plane / cache.tiles_in_height;
