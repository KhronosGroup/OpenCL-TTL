...
struct TTL_duplex_buffering {
     ...
     * \n\n Example:
     * @code
     * TTL_event events[2] = { TTL_get_event(), TTL_get_event()};
     *
    ...
    TTL_duplex_buffering(TTL_tensor<TENSORTYPE> ext_tensor_in, TTL_local(TENSORTYPE *) int_base_in,
        ...
        common.int_base [IMPORT_BUFFER] = int_base_in;
        common.int_base [EXPORT_BUFFER] = int_base_out;

        common.ext_tensor_in = ext_tensor_in;
        common.ext_tensor_out = ext_tensor_out;
        events = input_events;
        prev_out_tensors.to_export_to = TTL_tensor<TENSORTYPE>();
        prev_out_tensors.to_export_from = TTL_tensor<TENSORTYPE>();
    ...
    TTL_io_tensors<TENSORTYPE> step_buffering(TTL_tile tile_current_import, TTL_tile tile_current_export) {
        ...
        const TTL_sub_tensor<TENSORTYPE> next_import_int_sub_tensor(common.int_base [IMPORT_BUFFER],
                                                                tile_current_import.shape,
                                                                next_import_layout,
                                                                common.ext_tensor_in,
                                                                tile_current_import.offset);

        const TTL_tensor<TENSORTYPE> next_export_int_tensor = prev_out_tensors.to_export_from;
        const TTL_tensor<TENSORTYPE> next_export_ext_tensor = prev_out_tensors.to_export_to;

        if (tile_current_import.empty() == false)
            TTL_import_sub_tensor(next_import_int_sub_tensor,
                                  next_import_ext_tensor,
                                  &(*events) [IMPORT_BUFFER]);

        if (prev_out_tensors.to_export_from.empty() == false)
            TTL_export(next_export_int_tensor,
                       next_export_ext_tensor,
                       &(*events) [EXPORT_BUFFER]);

        const TTL_layout int_export_layout(tile_current_export.shape.width, tile_current_export.shape.height);
        const TTL_tensor<TENSORTYPE> to_export_to(common.ext_tensor_out.base,
                                                  tile_current_export.shape,
                                                  common.ext_tensor_out.layout,
                                                  tile_current_export.offset,
                                                  common.ext_tensor_out.elem_size);
        const TTL_sub_tensor<TENSORTYPE> to_export_from(common.int_base [EXPORT_BUFFER],
                                      tile_current_export.shape,
                                      int_export_layout,
                                      common.ext_tensor_in,
                                      tile_current_export.offset);

        prev_out_tensors.to_export_to = to_export_to;
        prev_out_tensors.to_export_from = to_export_from.tensor;
    ...
    TTL_common_buffering<TENSORTYPE, 2> common;  ///< The information that is common to all pipeline schemes
	...
	*/
    ...
    TTL_event (*events)[2];  ///< 2 Events are required, 1 first is used for
