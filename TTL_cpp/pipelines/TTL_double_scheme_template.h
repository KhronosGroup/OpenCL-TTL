...
typedef struct {
    ...
    TTL_tile prev_tile;                ///< Store of the previous imported/exported tile */
...
TTL_start_import_double_buffering(TTL_local(TTL_TENSOR_TYPE *) int_base1, TTL_local(TTL_TENSOR_TYPE *) int_base2,
                                  TTL_EXT_TENSOR_TYPE ext_tensor, TTL_event_t *event, TTL_tile first_tile);
#endif
...
__TTL_TRACE_FN(TTL_step_buffering, TTL_DOUBLE_BUFFERING_TYPE *const db, const TTL_tile next_tile);
