...
bool TTL_double_buffering(TEST_TENSOR_TYPE *restrict ext_base_in, int external_stride_in,
    ...
    for (int i = 0; i < TTL_number_of_tiles(input_tiler); ++i) {
        TTL_tile tile_next_import = TTL_get_tile(i + 1, input_tiler);
        TTL_tile tile_current_export = TTL_get_tile(i, output_tiler);

        TTL_sub_tensor imported_to = import_db.step_buffering(tile_next_import);
        TTL_sub_tensor exported_from = export_db.buffering(tile_current_export);

        compute(imported_to, exported_from);
    }

    import_db.finish_buffering();
    export_db.TTL_finish_buffering();
