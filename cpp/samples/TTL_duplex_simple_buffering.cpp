...
bool TTL_duplex_simple_buffering_kernel(TEST_TENSOR_TYPE *restrict ext_base_in, int external_stride_in,
    ...
    const TTL_tiler input_tiler(TTL_shape( width, height ), TTL_shape( tile_width, tile_height ));

    // Logical output tiling.
    const TTL_tiler output_tiler({ width, height }, { tile_width, tile_height });

    const TTL_tensor ext_input_tensor(ext_base_in, { width, height }, TTL_layout(external_stride_in));
    const TTL_tensor ext_output_tensor(ext_base_out, { width, height }, TTL_layout(external_stride_out));
    ...
    for (int i = 0; i < input_tiler.number_of_tiles(); ++i) {
        ...
        compute(ComputeType::COPY, tensors.imported_to, tensors.to_export_from);
    }

    duplex_scheme.finish_buffering();

    return result_check(ComputeType::COPY,                         ext_base_in,
						EXTERNAL_STRIDE_IN,
